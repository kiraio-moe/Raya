...
public class SerializableScenePropertyDrawer : PropertyDrawer
{
    ...
    private SerializedProperty sceneIndexInBuildSettings;
    private SerializedProperty isSceneEnabledInBuildSettings;
    ...
    private void CacheProperties(SerializedProperty property)
    {
        ...
        sceneIndexInBuildSettings = property.FindPropertyRelative("_sceneIndexInBuildSettings");
        isSceneEnabledInBuildSettings = property.FindPropertyRelative("_isSceneEnabledInBuildSettings");
    ...
    private void UpdateSceneState()
    {
        if (sceneAsset != null)
        {
            ...
            sceneIndexInBuildSettings.intValue = -1;
            for (int i = 0; i < scenes.Length; i++)
            {
                if (scenes[i].guid.ToString() == sceneAssetGUID)
                {
                    if(sceneIndexInBuildSettings.intValue != i)
                        sceneIndexInBuildSettings.intValue = i;
                    isSceneEnabledInBuildSettings.boolValue = scenes[i].enabled;
                    if (scenes[i].enabled)
                    {
    ...
    private void DisplaySceneErrorPrompt(string message)
    {
        ...
        if (choice == 0)
        {
            int newCount = sceneIndexInBuildSettings.intValue < 0 ? 
            ...
            if (sceneIndexInBuildSettings.intValue < 0)
            {
                newScenes[scenes.Length] = new EditorBuildSettingsScene(
                    sceneAssetPath, true);
                sceneIndexInBuildSettings.intValue = scenes.Length;
            }
            
            newScenes[sceneIndexInBuildSettings.intValue].enabled = true;
    ...
    private Rect DisplayErrorsIfNecessary(Rect position)
    {
        ...
        else if (!isSceneEnabledInBuildSettings.boolValue)
        {
